...
run_toolbox_remote "tailscale status --json 2>/dev/null | python3 -c 'import json,sys; data=json.load(sys.stdin); print(\"backend_state=\" + data.get(\"BackendState\", \"unknown\")); print(\"dns_name=\" + data.get(\"Self\", {}).get(\"DNSName\", \"unknown\").rstrip(\".\")); raw=json.dumps(data); print(\"tailnet_route_visible=\" + (\"yes\" if \"10.4.0.0/24\" in raw else \"no\"))' || echo backend_state=unavailable"
run_toolbox_remote "tailscale debug prefs 2>/dev/null | python3 -c 'import json,sys; data=json.load(sys.stdin); routes=data.get(\"AdvertiseRoutes\", []); print(\"route_advertised_local=\" + (\"yes\" if \"10.4.0.0/24\" in routes else \"no\"))' || echo route_advertised_local=unknown"
