...
- Toolbox network base: Caddy on \`10.4.0.20:80\`, AdGuard Home on \`10.4.0.20:53\` and localhost-only admin on \`127.0.0.1:3000\`, \`hs27.internal\` rewrites verified in opt-in mode
...
- Toolbox Tailscale state: \`/dev/net/tun\` mapped, \`tailscaled\` active, backend \`Running\`, subnet route \`10.4.0.0/24\` is advertised, and the remaining Split-DNS operator step is to point the restricted nameserver for \`hs27.internal\` to \`100.82.26.53\`
...
- Home Assistant OS is stable on \`10.4.0.24:8123\` and \`ha.hs27.internal\` now returns \`HTTP 200\` through Caddy
...
- Media server V1 is now live on \`CT 100 toolbox\`: Jellyfin is reachable internally on \`http://media.hs27.internal\`, directly on \`http://10.4.0.20:8096\`, and through the mobile Tailscale frontdoor on \`:8449\`; the obsolete local bootstrap sync is retired and Jellyfin reads from the central SMB-backed media path
