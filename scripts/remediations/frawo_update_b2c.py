...
print("Homepage updated (B2C Pivot).")

update_view(3637, contact_arch)
print("Contact page updated (Error Fix & B2C).")

models.execute_kw(db, uid, password, 'website', 'write', [[1], {'custom_code_head': global_css}])
print("Global CSS updated (Dark Mode Sync).")

# Fix Weissensberg in Footer (View 3632)
v = models.execute_kw(db, uid, password, 'ir.ui.view', 'read', [[3632]], {'fields': ['arch_db']})
f_arch = v[0]['arch_db']
if 'Weissensberg' in f_arch or 'Wei' + chr(65533) in f_arch:
    f_arch = f_arch.replace('Weissensberg', 'Wei' + chr(223) + 'ensberg')
    f_arch = f_arch.replace('Wei' + chr(65533), 'Wei' + chr(223))
    update_view(3632, f_arch)
    print("Footer corrected (Umlaut fix).")

print("All B2C Pivot updates complete!")
