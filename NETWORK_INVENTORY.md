...
| `10.1.0.20` | `toolbox` | `BC:24:11:AC:BD:44` / Proxmox | lxc admin toolbox | `infra-services` | homeserver-gbr | ssh + http + dns + vpn | static | high | active | pct + ansible + toolbox network check + tailscale + portal pilot preflight + 2026-04-07 firewall/runtime check | productive frontdoor currently runs as Docker container `toolbox-network_caddy_1`; `homeserver2027-toolbox-mobile-firewall.service` is active and enforces the Tailscale-only mobile ports |
