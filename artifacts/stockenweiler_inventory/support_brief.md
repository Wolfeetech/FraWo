...
- `home.prinz-stockenweiler.de` -> canonical `yourparty.tech` / `91.14.44.20` / error `none`
- `pve.prinz-stockenweiler.de` -> canonical `-` / `-` / error `dns_fail`
- `papierkram.prinz-stockenweiler.de` -> canonical `papierkram.prinz-stockenweiler.de` / `80.134.168.100` / error `timeout`
- `cloud.prinz-stockenweiler.de` -> canonical `yourparty.tech` / `91.14.44.20` / error `tls_error`
...
- `https://home.prinz-stockenweiler.de` -> `home_assistant` / title `Home Assistant` / login `unknown`
  - The Home Assistant frontend loads, but it immediately shows 'Unable to connect to Home Assistant. Retrying in ... seconds...', which means the web frontend answers while the backend service appears unavailable.
- `https://papierkram.prinz-stockenweiler.de/dashboard` -> `timeout` / title `N/A` / login `unknown`
  - Browser-visible state is ERR_CONNECTION_TIMED_OUT. The host does not currently answer on port 443.
- `https://cloud.prinz-stockenweiler.de/apps/dashboard/` -> `tls_error` / title `N/A` / login `unknown`
  - Browser-visible state is ERR_SSL_UNRECOGNIZED_NAME_ALERT, which indicates a missing or wrong certificate/SNI mapping at the reverse proxy.
- `https://pve.prinz-stockenweiler.de` -> `dns_fail` / title `N/A` / login `unknown`
  - Browser-visible state is ERR_NAME_NOT_RESOLVED. The hostname does not currently resolve.
