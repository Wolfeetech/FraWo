...
- `surface_laptop_start`: `failed` - url=`http://portal.hs27.internal/franz/` status=`200` final_url=`http://portal.hs27.internal/franz/` title=``
...
- `paperless`: `passed` - url=`http://paperless.hs27.internal/accounts/login/` status=`200` final_url=`http://paperless.hs27.internal/accounts/login/` title=``
