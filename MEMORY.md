...
- 2026-04-09: Der Odoo-Intake fuer `agent@frawo-tech.de` wurde auf `VM 200 nextcloud` real ausgerollt. Architektur: STRATO -> `webmaster@...` -> Alias-Router -> `Aliases.Agent` -> `odoo_agent_intake_bridge.py` -> Odoo-Task -> `Aliases.Agent.Processed`. Service/Timer live: `hs27-odoo-agent-intake.service` und `hs27-odoo-agent-intake.timer`.
- 2026-04-09: Der API-Key-Pfad fuer `agent@` war in dieser Session serverseitig erzeugbar, aber nicht belastbar fuer echten XML-RPC-Login. Produktiver V1-RPC-Pfad deshalb vorerst ueber dediziertes bot-only Secret ausserhalb des Repos statt ueber menschliche Zugangsdaten.
- 2026-04-09: Der Mediaserver sieht die zentrale Musik produktiv ueber `CT 100 toolbox:/srv/media-library/music-network` von `//10.1.0.30/Media`; `yourparty_Libary` ist damit live fuer Jellyfin erreichbar.
- 2026-04-09: `Wolf.EE` wurde read-only unter `/mnt/wolf-ee` gemountet und als Sortierquelle verwendet. Der erste Import in den zentralen Review-Pfad `/mnt/data/media/yourparty_Libary/incoming/Wolf_EE_20260409` umfasst `Job Jobse`, `The_TraXx`, `Sets` und `MUSIK` (ca. `14G`).
- 2026-04-09: Nach dem Wolf.EE-Import steht `CT 110 storage-node` bei ca. `91%` Belegung; weitere groessere Medienimporte in diesem Block pausieren.
- 2026-04-09: `Stockenweiler` blieb fuer Medienimport blockiert, weil von `toolbox` aktuell keine Route / kein Ping nach `192.168.178.25` besteht.
- 2026-04-09: Medien-/Kapazitaetsklarstellung. `Stockenweiler` ist nicht generell offline: Tailscale-Peer `stockenweiler-pve` (`100.91.20.116`) antwortet, aber der Importpfad ins LAN `192.168.178.0/24` fehlt auf `toolbox`, waehrend `tailscale status` weiter `--accept-routes is false` meldet. Der vorgesehene Entlastungspfad `/mnt/music_ssd` (`/dev/sdb1`, exfat) ist wegen Dateisystemfehlern read-only; `dmesg` meldet `exFAT-fs ... Filesystem has been set read-only`. Praktische Folge: `storage-node` bei ca. `91%` bleibt der aktuelle Medien-Druckpunkt, keine weiteren grossen Importe bis Entlastung oder SSD-Reparatur.
