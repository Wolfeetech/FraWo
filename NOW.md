...
>   - **Öffentliche Registrierung sofort geschlossen:** `auth_signup_uninvited` auf Website 1 stand auf `b2c` (jeder konnte Konten anlegen!). Auf `b2b` umgestellt und `auth_signup.invitation_scope` auf `b2b` gesetzt. Direkte Aufrufe von `/web/signup` antworten jetzt mit 404 Not Found, Registrierungslink auf `/web/login` komplett entfernt.
>   - **Spam-Account bereinigt:** Unbefugt registrierter Account #28 (`madamtaisia@mail.ru`, vom 16.09.) sowie Test-Account #29 (`test@test.de`) deaktiviert (`active=False`).
>   - **Cloudflare Real-IP Rekonstruktion in Nginx (CT140):** Vollständige Cloudflare IP-Ranges (IPv4/IPv6) via `set_real_ip_from` hinterlegt, `real_ip_header CF-Connecting-IP` aktiviert. Verifiziert: Nginx sieht und loggt jetzt die echte Client-IP (`92.211.33.82`) statt der internen Docker-Bridge (`172.18.0.2`).
>   - **AI-Bot-Sperre & Rate Limiting an der Reverse-Proxy-Kante:**
>     - AI-Scraper Map für User-Agents (`GPTBot`, `ClaudeBot`, `PerplexityBot`, `Bytespider`, etc.) sperrt `/web/login`, `/web/session/authenticate`, `/web/signup`, `/xmlrpc` und `/jsonrpc` sofort mit **403 Forbidden** (`Forbidden: Automated AI bot access to login is blocked.`). Live verifiziert via curl.
>     - Rate Limiting (`limit_req_zone $binary_remote_addr zone=login_limit:10m rate=5r/m burst=5 nodelay`) implementiert. Live bewiesen via curl: Requests 1–6 passieren, Request 7 & 8 werden sofort mit **429 Too Many Requests** abgewiesen.
>     - Exploit-Scanner (.php, .env, .git, wp-login, phpmyadmin) werden sofort mit 404 abgewiesen.
>     - Nginx-Konfiguration im Repo versioniert unter `deployments/frawotech-web/nginx.conf`.
> - **Speicher- und Host-Resilienz (stock-pve & anker-pve):**
>   - **Toter CIFS-Mount auf VM 210 (AzuraCast) bereinigt:** Veralteter Eintrag `//10.1.0.94/radio` in `/etc/fstab` entfernt (Samba-Share war seit 04.09. stillgelegt). `mnt-radio_rw.mount` und HW-Treiber `openipmi.service` deaktiviert. `systemctl --failed` auf VM 210 jetzt **0 Fehler**.
>   - **Doppelter `node_exporter` auf stock-pve bereinigt:** Eigenständiger `node_exporter.service` kollidierte auf Port 9100 mit Debian-Paket `prometheus-node-exporter.service`. Redundanter Service entfernt. `systemctl --failed` auf stock-pve jetzt **0 Fehler**.
>   - **Rclone-Watchdog auf Anker stabilisiert:** Neuer permanenter Service `deployments/systemd/rclone-gdrive-v2.service` mit `TimeoutStopSec=30` und `KillMode=mixed`. `/usr/local/bin/rclone-gdrive-watchdog.sh` erkennt gesunden Mount und v2-Service. Watchdog meldet `healthy` und läuft ohne Endlos-Restartschleife.
>
> **Am 17.09.2026 abends repariert & verifiziert (Antigravity):**
> - **Stromabrechnung Stockenweiler saniert (Odoo #1487):** Monatszähler waren um das bis zu 500-fache zu hoch (Lotti 20.713 kWh, Container 19.502 kWh, Familie Prinz 38.515 kWh), weil `utility_meter` auf Gruppensensoren mit `total_increasing` lag und jeder Shelly-Verbindungsabbruch als Zählerüberlauf interpretiert wurde. Repariert: 16 entkoppelte Einzel-Zähler in `configuration.yaml`, stabile Summen in `templates.yaml` mit `state_class: total`. Falscher Watt-Zähler `bkw_container_daily_035` entfernt. Echte September-Verbräuche aus der DB rekonstruiert und kalibriert: **Lotti 42,27 kWh**, **Container 0,74 kWh**, **Familie Prinz 119,35 kWh**. `ha core check` sauber, alle Zähler sammeln inkrementell.
> - **Odoo-Cloud-Sicherung v2 entkoppelt & verifiziert (Odoo #1481):** Alter Service hing in `failed (timeout)`, weil zwei historische `mkdir`-Prozesse im Kernel-D-State der Cgroup feststeckten und SIGKILL ignorierten. `frawo-odoo-cloud-v2.service` und `.timer` (07:35 CEST) angelegt. Frischer Dump (153 MB, `FraWo_GbR_20260917_171137.sql.gz`) erzeugt, verschlüsselt nach `gcrypt:Odoo` übertragen und am Ziel per `rclone` bestätigt. `frawo_odoo_backup_offsite_ok 1`, Metriken aktuell, Service sauber mit Exit 0 beendet. Nebenbei `rclone-gdrive-watchdog.sh` korrigiert, der mangels Erkennung von `rclone-gdrive-temp.service` alle 30 min in eine Endlos-Restartschleife lief.
> - **FraWo 2.0 Gesamtkonzept verankert (`DOCS/FRAWO_2_0_GESAMTKONZEPT.md`):** Wolfs Direktive umgesetzt: FraWo 2.0 ist das Gesamt-Upgrade für alle Server/Infrastruktur (nicht nur Radio). Vier Säulen: 1. Login-Sicherheit & AI-Bot-Sperre (Turnstile / WAF / Rate-Limits / Fail2ban), 2. Speicher-Resilienz (Timeouts, D-State-Bann, `/var/tmp`), 3. Alarm-Hygiene & Stille (Selbstheilung → 12:04 Lagebericht → Critical only per Telegram), 4. Dienst-Modularität (Entflechtung View 3353, Wildcard-TLS).
>
> **🔴 Am 17.09.2026 nachts gelernt — zwei Fallen, die Zeit gekostet haben:**
> - **Ein Wächter darf nie an dem hängen, was er absichern soll.** Wolfs Licht brannte fünf Stunden durch (00:08–05:10). Die Kette: Die Automatik schaltete ein und setzte ihren Besitzmerker; **elf Sekunden später** lief der Zweig „Benutzer hat eingegriffen", weil die Govee-MQTT-Brücke erst **eine** der beiden Lampen als `on` zurückgemeldet hatte — ein halb gemeldeter Gruppenzustand sieht aus wie ein Benutzereingriff. Besitz abgegeben, und der Ausschaltzweig setzt genau diesen Merker voraus. **Derselbe Fehler dann nochmal 40 Minuten später** im neu gebauten NOTAUS („Pause ist aus" als Bedingung — und die Pause stand an). Regel: Fangnetz-Bedingungen **nur aus Messwerten und Gerätezuständen**, nie aus selbst geführten Merkern. Und Ist/Soll-Vergleiche über Funkbrücken brauchen eine **Karenzzeit** (hier 90 s). Odoo #1475.
> - **HA liefert alle Zeitstempel in UTC** — `/api/history/period`, `last_changed`, Traces. Als Ortszeit gelesen sind das im Sommer **zwei Stunden** Versatz. Ich habe daraus einen „ungeklärten Tag/Nacht-Fehler" der Lichtautomatik konstruiert, den es **nie gab** (22:07 UTC = 00:07 Ortszeit, da gilt der Nachtwert völlig korrekt), und über eine Stunde verfolgt. Gegenprobe kostet Sekunden: ein Ereignis nehmen, dessen Ortszeit man selbst gesehen hat. Dritter Zeitzonenfehler nach Odoo-SQL und systemd-Timern.
>
> **Am 16.09.2026 — Home Assistant aufgeräumt (beide Anlagen), Odoo #1475:**
> - **Ortszuordnung ist jetzt belegt, nicht geraten.** 🔴 Die Annahme „kommt über `remote_homeassistant` ⇒ steht in Stockenweiler" ist **falsch** — die Eltern-Anlage (`10.1.0.248`) läuft physisch in Rothkreuz und verwaltet auch Rothkreuzer Geräte. **Der belastbare Weg ist die IP:** `10.4.0.x` = Rothkreuz (FrawoIoT), `192.168.178.x` = Stockenweiler (Alopri). Zwei Namen führten nachweislich in die Irre: „Routerschrank" steht auf `192.168.178.62` (Elternhaus, WLAN-Technik), und die kritische IT-Schiene `10.4.0.11` hiess **„Outdoor Garage"** bzw. im Gerät **„Werkstatt"**. Alles umbenannt nach Schema **`Ort · Bereich · Funktion`** (30 Geräte, nur Anzeigenamen — Entity-IDs unangetastet).
> - **Bei WLAN-Geräten schlägt die BSSID die IP.** Es gibt im ganzen Netz **genau einen** Zugangspunkt („AC Mesh" U7MSH, `FraWo__ioT` auf `7a:ac:b9:6d:79:5a`). Alle IoT-Shellys hängen daran — ein Gerät in der Villa (Rothkreuz 14, Franznetz) wäre dort gar nicht sichtbar. **Rothkreuz 14 hat keine Netzabdeckung**; vor jedem Studio-Ausbau dort ist das der erste Schritt.
> - **276 Karteileichen abgeschaltet, nichts gelöscht.** Rothkreuz 1418 → **1252** Entitäten (tot 427 → 259), Stockenweiler 1051 → **941** (tot 232 → 126), kaputte Integrationen dort 5 → 1. 🔴 **Prüfkriterium war NICHT „unavailable"** (so gingen am 07.09. 48 lebende Entitäten verloren), sondern: *existiert die Quelle drüben überhaupt noch?* Ergebnis: von 411 toten Entitäten waren nur **168** wirklich verwaist — ein pauschales „alles Tote weg" hätte **243 lebende** Geräte mitgerissen. Rückrolllisten vorhanden, alles über `disabled_by=user`.
> - ⚠️ **Stolperstein: `disabled_by` wirkt erst beim vollständigen Neustart.** Bei `remote_homeassistant` und `fritz` ändert ein `reload` des Konfigurationseintrags die Registry, aber **nicht den Betrieb** — zweimal unabhängig gemessen (152 bzw. 94 Entitäten blieben stehen, nach dem Neustart 0). Immer die Entitätenzahl nachmessen.
> - **Zwei Dashboards mit identischem Inhalt** („Übersicht" + „Kiosk") liefen auseinander; jede Korrektur wirkte nur auf einem. `kiosk_dashboard.yaml` enthält jetzt nur noch `!include ui-lovelace.yaml` — eine Quelle, zwei Zugänge, `show_in_sidebar: false`.
> - **Lichtregelung statt Lichtschalter** (4 Govee-Lampen Rothkreuz): regelt auf eine Wunschhelligkeit im Raum (Schieberegler 60 lx tags / 20 lx nachts), sanfte Übergänge, Schritte von 10 Punkten alle 2 min, Totzone gegen Aufschaukeln. 🔴 **Jarvis' Review deckte einen unbegrenzten Dauerlauf auf**: Fiel der Bewegungsmelder aus oder startete HA neu, feuerte der Ausschalter nie. Jetzt Sicherheitszweig (Melder unbrauchbar oder >5 min ohne Bewegung → aus) plus 2-Stunden-Obergrenze, live bewiesen. **Offen:** echter Dunkeltest, erst nachts messbar.
> - **Tapo-Kamera** (`10.4.0.35`) über die **eingebaute ONVIF-Integration** eingebunden (Port 2020, lokales Kamerakonto von Wolf) — die HACS-Erweiterung „Tapo: Cameras Control" war entgegen früherer Notiz **nie installiert**. Standbild und Livestream verifiziert.
> - ⚠️ **Falschmessung, die fast zur Firewall-Suche geführt hätte:** `qm guest exec 210 -- /bin/bash` läuft in HAOS **nicht** als bash (`$BASH_VERSION` leer) → `/dev/tcp` existiert nicht → **jeder** Port sieht geschlossen aus. Verlässlich: `docker exec homeassistant python3` mit echtem Socket.
> - **Bereiche (Räume) eingeführt:** vorher hatten **76 von 114** Geräten gar keinen Raum. Neu: Badezimmer, Flur, Vor der Tür, Technik, Studio (Rothkreuz 14) — die letzten fünf **noch leer**, dort ist schlicht keine Hardware. Offen: #1483 (Standorte/Personen), #1484 (3D-Räume, Idee).
>
...
> - **Cloud-Sicherung des Ankers lief still ins Leere — repariert.** Der rclone-Zwischenspeicher lag auf `/anker-backup`, also auf dem suspendierten Pool: dadurch blockierte *jeder* Zugriff auf `/mnt/google-drive`, der 04:00-Lauf hing 5,5 h beim ersten von zehn Gästen, und alle drei Sicherungswege waren gleichzeitig tot. Cache auf `/var/cache/rclone` (25 G statt 120 G). Der hängende `vzdump` stand im D-Zustand und hielt `/var/run/vzdump.lock` — gelöst über gezielten Abbruch der toten FUSE-Verbindung (`/sys/fs/fuse/connections/<nr>/abort`, gesunde Verbindung vorher per `stat -c %d` identifiziert). Bewiesen mit echter Sicherung (CT101, 255 MB, Datei am Ziel geprüft). **Bestand vom 14.09. ist vollständig** (alle 10 Gäste). ⚠️ Übergangszustand: Die Dauer-Unit hängt hinter einem Zombie in `deactivating`, die Einbindung läuft als `rclone-gdrive-temp.service`; beim nächsten Neustart übernimmt wieder die korrigierte Dauer-Unit. Odoo #1462.
> - **Jarvis war tot und läuft wieder.** Alle drei Modelle liefen über GitHub Copilot, dessen Kontingent erschöpft war (`429`) — ein Anbieter, Totalausfall. Jetzt über Wolfs **ChatGPT/Codex-Abo** (OAuth, kein API-Key): Standard `openai/gpt-5.6-sol`, Reservekette über **zwei** Anbieter. Update **2026.9.2 → 2026.9.4** (verlangt Node ≥ 24.16, Abbild auf `node:24-slim` angehoben); `OPENAI_API_KEY` enthielt fälschlich ein GitHub-Token und wurde entfernt. Jeweils mit echtem Agentenlauf verifiziert. Odoo #1463. ⚠️ Modell-IDs immer aus `openclaw models list` des Kontos nehmen — die Doku nannte eine ID, die das Konto nicht freigibt.
>
> **Bewusst aufgeschoben:** System-Updates auf beiden Proxmox-Rechnern (Anker 1–2 Pakete, stock-pve **99**, beide mit Neustart-Bedarf). Erst wenn die USB-Platten zurück sind und der Sicherungs-TÜV grün läuft — Updates ohne funktionierendes Netz sind die falsche Reihenfolge, und ein Neustart des Ankers kann derzeit am suspendierten Pool hängenbleiben.
>
...
> - **Musikbibliothek (464 GB) war nirgends gesichert** — Bind-Mounts fallen aus vzdump heraus. Nächtlicher Abgleich auf den Anker gebaut samt eigener TÜV-Prüfung; **pausiert**, bis die Anker-Hardware geklärt ist. Odoo #1457.
> - **Plattentemperaturen jetzt messbar** (vorher blind): Musikplatte läuft bei **65 °C**, Familiendaten 60 °C — die Platten müssen aus Platzgründen gestapelt werden. Odoo #1460.
> - **Dauerhafte Protokollierung auf stock-pve eingeschaltet** — bis dahin löschte jeder Absturz seine eigene Spur.
>
...
| **`openclaw config set …` löste bis 2026.7.x IMMER einen vollständigen Gateway-Reload aus** | Der Reload las **alle** Schlüssel neu — auch `agents.defaults.model`. Wer nur Telegram/exec härtete, aktivierte damit ein evtl. gerade eingetragenes, kaputtes Modellprofil mit. So am 06.09.2026 Jarvis 5 Minuten stumm geschaltet | ✅ **Behoben mit dem Update auf 2026.9.2 (07.09.2026).** `config set` meldet jetzt „Change will apply without restarting the gateway", im Betrieb über drei Änderungen bestätigt. Modellnamen weiterhin nie aus dem Gedächtnis eintragen — Test: `docker exec openclaw openclaw agent -m "Antworte nur mit OK" --session-key check` (ohne `--deliver`) |
| **OpenClaw-Update: Gateway startet danach nicht mehr** | Meldung `Legacy workspace setup state requires migration for /root/.openclaw/workspace; run openclaw doctor --fix`. Der Container läuft, aber Port 19000 lehnt ab — `openclaw health` sagt `ECONNREFUSED`. Der Reparaturbefehl braucht einen **gestoppten** Gateway, im laufenden Container läuft er also ins Leere | `docker stop openclaw`, dann Einweg-Container auf demselben Volume: `docker run --rm --network host --env-file /opt/openclaw/.env -v openclaw_openclaw-data:/root/.openclaw --entrypoint openclaw openclaw-openclaw:latest doctor --fix`, danach `docker compose up -d`. **Anschließend startet Telegram nicht automatisch** („crash-loop breaker") → von Hand: `openclaw gateway call channels.start --params '{"channel":"telegram"}'` |
| **Modellnamen: Punkt bei Copilot, Bindestrich bei Anthropic** | `github-copilot/claude-haiku-4.5` und `github-copilot/claude-opus-4.8` — bei Anthropic dagegen `claude-haiku-4-5`. Ein falsches Zeichen ergibt ein stumm nicht ladendes Modell | Vor dem Eintragen `openclaw models list \| grep copilot` und den Namen kopieren |
...
| 🔴 **`net-resilience.sh` kappt bei jedem Ping-Fehler alle Gäste** | Eigenes Cron-Skript auf dem Anker (`/etc/cron.d/net-resilience`, **alle 5 Minuten seit Okt. 2025**): pingt das Gateway, und bei Fehlschlag `systemctl restart networking` — Proxmox baut dabei `vmbr0` neu auf und nimmt **nur die konfigurierten Ports** mit. Alle Gast-Schnittstellen bleiben draußen und kommen **nie von selbst zurück**. Am 07.09.2026 um 19:50:00 reichte ein **3 Sekunden langer Wackler** der physischen Karte: Website, Odoo, Vaultwarden, Paperless und alle VMs waren **12 Stunden weg**, während der Link längst wieder oben war. `/var/log/net-resilience.log` zählt **579** solcher Neustarts. Zweite Stufe des Skripts ist ein **harter Reset** (`echo b > /sysrq-trigger`) — der zündet nur deshalb nie, weil der Pfad falsch ist (richtig wäre `/proc/sys/kernel/sysrq-trigger`); stattdessen entsteht eine Datei `/sysrq-trigger`, der Befehl meldet Erfolg und das `\|\| reboot -f` entfällt. Die Datei existiert seit dem 05.07.2026, an dem Tag hat der Reset-Zweig **dreimal** gefeuert | **Cron-Job abschalten**, Skript nicht „reparieren": `mv /etc/cron.d/net-resilience /root/net-resilience.cron.deaktiviert-YYYYMMDD`. Ein Netzwerk-Neustart ist auf einem Virtualisierungswirt nie die Antwort auf einen Ping-Fehler. Symptom-Erkennung: `ls /sys/class/net/vmbr0/brif/` — steht dort nur `nic0`, sind alle Gäste abgekoppelt. Sofort-Reparatur ohne Neustart: `for d in veth<id>i0 … fwpr<id>p0 tap<id>i0; do ip link set $d master vmbr0; done` |
...
| 🔴 **systemd-Timer rechnet in der Zeitzone des WIRTS, Container-Cron in UTC** | Der Odoo-Abzug in CT140 läuft per Cron `30 3 * * *` — der Container steht auf **UTC**, das sind **05:30 Ortszeit**. Der am 08.09.2026 gebaute Timer für die verschlüsselte Cloud-Kopie stand auf `OnCalendar=04:30`, und systemd rechnet in **CEST** — also 02:30 UTC, **drei Stunden vor dem Abzug**. Der Kopierlauf nahm deshalb jeden Tag klaglos die Datei vom **Vortag**, meldete Erfolg und setzte `frawo_odoo_backup_offsite_ok 1`. Die Prüfung „Datei am Ziel angekommen?" war korrekt und grün — nur eben für den falschen Tag. Verdeckt wurde es zusätzlich durch `MAXALTER_STUNDEN=26`: die 23 Stunden alte Datei ging anstandslos durch. Im Ernstfall hätte bis zu 48 Stunden Arbeit gefehlt | Timer auf **06:15 Ortszeit** (nach dem Abzug) und **`MAXALTER_STUNDEN=8`** — fehlt der frische Abzug, bricht der Lauf jetzt **laut** ab, statt still den alten zu kopieren. **Regel:** Bei jedem Zeitplan, der auf einen anderen Zeitplan folgen soll, beide Zeitzonen ausdrücklich prüfen (`date`/`date -u` auf **beiden** Seiten). Und: Eine Altersgrenze, die größer als der Abstand zweier Läufe ist, macht den Frischetest blind. Gefunden 09.09.2026 im eigenen Protokoll des Skripts (`Abzug gefunden: …_20260908_…, 23 h alt`) |
| 🔴 **Odoo-Stufen hängen als many2many an Projekten — ein Projekt ohne Zuordnung kippt alles in eine Spalte** | Der Kanban wirkte als „kompletter Wildwuchs". Ursache war nicht Unordnung durch Benutzer, sondern eine **lückenhafte Stufenzuordnung**: Von 33 vorhandenen Stufen trugen nur **7** überhaupt Aufgaben, und die waren ungleich verteilt — Projekt 163 (Business) kannte nur `🚀 In Arbeit`, Projekt 159 (Studio) nur `⚙️ In Planung`, Projekt 110 drei Stufen. Alles, was dort landete, stapelte sich zwangsläufig in einer einzigen Spalte. Die übrigen 26 Stufen (`Canceled` ×6, `Inbox`, `To-do`, `Heute`, `Diese Woche` …) waren leer und hingen nur an archivierten Projekten. **Derselbe Mechanismus wie bei den 62 Aufgaben, die am selben Tag ihre Stufe verloren** — dort fehlte die Zuordnung beim Zielprojekt, hier bei fast allen | Am 09.09.2026 vereinheitlicht: die 7 echten Stufen an **alle 11 aktiven Projekte** gehängt, feste Reihenfolge `💡 Idee(0) → 📋 Backlog(1) → ⚙️ In Planung(2) → 🚀 In Arbeit(3) → 🛑 Blockiert(4) → ✅ Erledigt(5, gefaltet) → 🗑️ Abgebrochen(6, gefaltet)`. **Prüfabfrage, die das findet:** Aufgaben zählen, deren `stage_id` nicht über `project_task_type_rel` mit ihrem `project_id` verbunden ist — die sind im Kanban unsichtbar. Ergebnis danach: 0 (die 2 Treffer gehören Projekt 64, einer **anderen Firma** in derselben Datenbank — deshalb im FraWo-Kanban ohnehin nicht sichtbar) |
...
| 🔴 **Ein Shelly-Zeitplan auf der falschen Steckdose legt täglich die IT lahm** | Auf `10.4.0.11` („IT/Netzwerk-Strom KRITISCH") lagen zwei Zeitpläne **im Gerät**: `@sunrise` → ein, **`@sunset` → aus**. Gedacht waren sie für die GrowBox (`10.4.0.13`), dort landeten sie nie — die hat ihre eigenen. Weil `auto_on` auf **0,01 s** steht (bewusst so), schaltet sich die Dose 10 ms später selbst wieder ein: ein **täglicher Stromausfall von 10 Millisekunden** auf der IT-Schiene. Zu kurz zum Bemerken, lang genug, um laufende Server abstürzen zu lassen und bus-versorgte USB-Platten abzuwerfen. Beleg 14.09.2026: Sonnenuntergang 19:35–19:45, ProDesk-Neustart laut `last -x` um **19:36**; Zähler der Dose `switch_on=9` bei 8 Tagen Laufzeit = rund einer pro Tag | Zeitpläne **am Gerät** prüfen, nicht in Home Assistant: `curl http://<ip>/rpc/Schedule.List`. Gerätetimer überstimmen HA lautlos. Auf `.11` gehört **kein** `@sunset`-Job. `auto_on` unangetastet lassen — das ist das Sicherheitsnetz, kein Fehler. Odoo #1461 |
| 🔴 **Prometheus überwachte den Alertmanager nicht — 96 % aller Alarme kamen nie an** | 17 Ziele wurden gescrapt, darunter Prometheus selbst und der Windows-Arbeitsplatz, **aber nicht der Alertmanager**. Dessen eigene Zähler standen bei `notifications_failed_total{telegram} = 1617` von 1679 Versuchen und `{webhook} = 264` von 318 — sichtbar nur, wenn man sie von Hand abruft. Ohne Scrape gibt es keine Historie, keine Regel und damit keinen Alarm darüber, dass die Alarme nicht ankommen. Zusätzlich schreibt der Dienst **keine Journal-Einträge**, die Fehlschläge hinterließen also nicht einmal eine Logspur | Am 14.09.2026 Scrape-Job `alertmanager` ergänzt und Regeln `AlarmzustellungScheitert` / `AlertmanagerNichtErreichbar` angelegt. **Regel:** Wer einen Melder baut, muss den Melder selbst messen — sonst ist Stille nicht von Ruhe zu unterscheiden. Odoo #1459 |
| 🔴 **Zwei Alertmanager-Konfigurationen — bearbeitet wurde die falsche** | Geladen wird `/etc/prometheus/alertmanager.yml` (Debian-Paketpfad). Daneben existiert `/etc/alertmanager/alertmanager.yml` mit einem sauber eingerichteten E-Mail-Empfänger, den nie jemand liest. Symptom: `alertmanager_notifications_total{integration="email"} = 0` — die Mail-Alarmierung sah fertig aus und war nie aktiv | Vor jeder Änderung prüfen, welche Datei der Prozess wirklich geladen hat: `/api/v2/status` liefert die **laufende** Konfiguration. Die Datei auf der Platte beweist nichts |
| **`inhibit_rules` mit `equal: ["alertname"]` kann prinzipbedingt nie greifen** | Die Regel sollte kritische Alarme die zugehörige Warnung stumm stellen lassen — verglich aber `alertname`. `ThinPoolUsageCritical` und `ThinPoolUsageWarning` heißen per Definition verschieden, also traf der Vergleich nie zu und beide kamen doppelt. Zusammen mit `group_by: ["alertname","instance"]` wurden aus 2 echten Problemen **15 Telegram-Nachrichten** — bis Wolf abgestumpft war | `equal: ["instance"]` (gemeinsames Merkmal statt Alarmname) und `group_by: ["alertname"]`. Ergebnis am 14.09.2026 nachgezählt: dieselbe Lage erzeugt **3 statt 15** Nachrichten. Und: `warning` gehört nicht aufs Handy, nur `critical` |
| 🔴 **Bind-Mounts sichert vzdump prinzipbedingt nicht mit** | CT120 wird jede Nacht erfolgreich gesichert — mit **7,6 GB**. Die 464 GB Musik hängen als `mp0: /mnt/music_hdd,mp=/mnt/music` im Container und sind in **keiner** Sicherung enthalten. Kein Fehler, keine Warnung, der Auftrag meldet Erfolg und übergeht die Daten still. Gleiche Familie wie 16.08., 20.08. und 07.09. | Sicherungsgröße gegen Quellgröße prüfen, nicht nur „ist der Auftrag gelaufen". Für Bind-Mounts einen eigenen Weg bauen (seit 14.09.2026: `frawo-musik-sync.timer` auf den Anker + eigene TÜV-Prüfung `musik_bibliothek`). Odoo #1457 |
| **journald ohne `/var/log/journal` — jeder Absturz löscht seine eigene Spur** | Auf stock-pve lief die Protokollierung nur im Arbeitsspeicher. Die Maschine ist mehrfach unsauber ausgefallen (26.08. zweimal, 07.09., 14.09.), und **jedes Mal waren die Protokolle nach dem Neustart weg** — `journalctl -b -1` liefert nichts. Ursachensuche war dadurch unmöglich | `mkdir -p /var/log/journal && systemd-tmpfiles --create --prefix /var/log/journal && systemctl restart systemd-journald`. Am 14.09.2026 auf stock-pve nachgeholt. Auf jedem Knoten prüfen, der schon einmal unerklärt neu gestartet ist |
...
> | Radio VM210 AzuraCast | ProDesk | ✅ **wieder online & live verifiziert** (14.09. 03:04, `10.1.0.38`, AutoDJ + Icecast + Cloudflare-Stream aktiv, Cron 51 reaktiviert) |
...
**Bildschirmtastatur am Touchscreen (06.09.2026, Odoo #1353):** `onboard` war installiert und lief, blendete sich aber **nie** ein. Ursache im Protokoll wörtlich: `Atspi typelib missing, auto-show unavailable` — das Paket `gir1.2-atspi-2.0` fehlte, ohne das kann die Tastatur Eingabefelder prinzipiell nicht erkennen und jede Einstellung bleibt wirkungslos. Zusätzlich fehlten drei Voraussetzungen: `org.onboard.auto-show enabled` stand auf `false`, `toolkit-accessibility` auf `false`, und Chromium lief ohne `--force-renderer-accessibility` (ohne das meldet Chromium seine Textfelder gar nicht erst). Ausserdem unterdrückte `tablet-mode-detection-enabled=true` das Einblenden auf einem Desktop-Rechner. Alles gesetzt, Design **Nightshade** (dunkel) mit Layout **Compact**, unten angedockt, `force-to-top` über dem Vollbild-Browser. Umlaute per Langdruck (a → ä, o → ö, u → ü, s → ß). Nach vollständigem Sitzungs-Neustart reproduziert. Backup: `/home/kiosk/.xinitrc.bak-20260906`.
