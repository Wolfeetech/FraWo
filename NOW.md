...
| **Komma innerhalb eines `feld::regex`-Werts** | `genre::^Techno,` sieht wie ein Ganzzeichen-Match aus, wird aber **am Komma aufgespalten** — die leere zweite Hälfte trifft **die gesamte Bibliothek** (bzw. verschmilzt mit der nächsten Bedingung zu einem ODER). Kostete am 04.09.2026 **5.437 Falschtags** in 4 Schritten eines Batch-Skripts (u.a. 178 statt 3.793 „raw-hypnotic-techno"-Treffer), inkl. Überschreiben bereits korrekter/EXCLUDED-Tags | Kommas in Mehrfach-Genre-Strings (`"Techno, Electronic"`) **nie** roh in eine Regex packen — Komma escapen (`\,`) oder auf den Teil vor dem Komma beschränken (`genre::^Techno$` bzw. ohne Komma). Jede neue Genre-Query zuerst isoliert mit `wc -l` gegen eine plausible Hausnummer prüfen |
...
| ~~`R:` abschalten~~ — **erledigt 04.09.2026.** Verwaiste AzuraCast-`storage_location` (id 9, `network_library`, 253 Medien, von keiner Station/Playlist referenziert) entfernt, danach `[radio]`-Freigabe in `smb.conf` auf CT120 auskommentiert (Sicherung `smb.conf.backup-20260904-radio-retired`), `smbd` neu gestartet. `\\10.1.0.94\radio` antwortet nicht mehr, `/` auf stock-pve bleibt bei 37 GB frei. Die alten ~6,9 GB unter `/mnt/stick/yourparty.radio` liegen noch auf der Wirt-Systemplatte (kein Risiko mehr, da nichts mehr reinschreibt) — kleiner Aufräumschritt für später | — | #1263 |
