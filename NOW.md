...
| **Paperless: `paperless_mail.tasks.process_mail_accounts` hängt fest → würgt die ganze Dokumenten-Pipeline ab** | Stratos IMAP-Server schließt die Verbindung vorzeitig (`* BYE Autologout; idle for too long`), der Task hängt dadurch bis zum Celery-Hard-Limit (1800 s), wird per `SIGKILL` beendet, `celery beat` setzt **alle 10 Minuten** einen neuen an — mit `docker exec paperless-webserver docker logs` sichtbar als endlose `TimeLimitExceeded`-Kette seit mind. 05.09. 23:01. Da nur wenige Celery-Worker laufen, **blockieren diese hängenden Mail-Tasks die `consume_file`-Tasks für neu ankommende Dokumente** — frisch von Drive gezogene Dateien blieben am 06.09. minutenlang auf `pending` stehen, obwohl der Consumer sie längst „File is stable" gemeldet hatte | Kurzfristig: `PATCH /api/mail_rules/1/ {"enabled": false}` schaltet den Auslöser ab, Dokumenten-Queue läuft sofort wieder frei. Noch offen: echte Ursache (IMAP-Timing zu Strato) beheben oder Rule dauerhaft deaktiviert lassen. Odoo #1285 |
